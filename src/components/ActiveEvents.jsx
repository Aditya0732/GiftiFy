...
import { IoCalendarOutline, IoLocationOutline, IoPersonOutline, IoAddCircleOutline, IoCashOutline, IoInformationCircleOutline, IoGiftOutline, IoSearchOutline, IoFilterOutline, IoTimeOutline, IoChevronBackOutline, IoChevronForwardOutline } from "react-icons/io5";
import EventDetailsModal from './EventDetailsModal';
import ContributionModal from './ContributionModal';
import { useSession } from 'next-auth/react';
import { motion } from 'framer-motion';
import { useAddContributionMutation } from '@/store/api';
...
export const ActiveEvents = ({ events }) => {
  ...
  const activeEvents = useMemo(() => events.filter(event => event.status !== 'closed'), [events]);
  const isUserHost = (event) => event.host._id === session?.user?.id;
  const { showSuccessToast } = useToast();

  const filteredEvents = useMemo(() => {
    let filtered = activeEvents.filter(event => {
      const matchesSearch = Object.values(event).some(value =>
        value.toString().toLowerCase().includes(searchTerm.toLowerCase())
      );
      const matchesFilter =
        filter === 'all' ||
        (filter === 'hosted' && isUserHost(event)) ||
        (filter === 'not-hosted' && !isUserHost(event));
    ...
    return filtered;
  }, [activeEvents, searchTerm, filter, timeSort, session]);
  ...
  return (
    <div className="mt-12 px-5 md:px-0 max-w-6xl mx-auto">
      <motion.h2
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <div className="inline-flex items-center bg-gray-100 rounded-full p-1">
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${filter === 'all' ? 'bg-purple-500 text-white shadow-md' : 'text-gray-700 hover:bg-gray-200'}`}
              >
                All Events
              </button>
              <button
                onClick={() => setFilter('hosted')}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${filter === 'hosted' ? 'bg-purple-500 text-white shadow-md' : 'text-gray-700 hover:bg-gray-200'}`}
              >
                Hosting
              </button>
              <button
                onClick={() => setFilter('not-hosted')}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${filter === 'not-hosted' ? 'bg-purple-500 text-white shadow-md' : 'text-gray-700 hover:bg-gray-200'}`}
              >
                Attending
              </button>
            </div>
          ...
          {paginatedEvents.map((event, index) => {
            ...
            return (
              <motion.div
                ...
                initial="hidden"
                animate="visible"
                className={`w-full bg-gradient-to-r ${userIsHost ? 'from-blue-400 to-purple-500' : 'from-green-400 to-blue-500'} rounded-lg shadow-lg overflow-hidden transition-all duration-300`}
              >
